    ...
    mutate(location_region = case_when(settlement %in% c("Kampala") ~ "Kampala",
                ...
                i.hoh_disability = ifelse(progres_relationshiptofpname %in% c("Focal Point") & i.hh_with_disabled_member %in% c("yes_disability"),
                  ...
                  # livelihoods indicators
                   ...
                   i.hh_with_child_outside_of_home_by_childlocation = case_when(progres_relationshiptofpname %in% c("Focal Point") & 
                                                                          3 %in% c(1699:1703) ~ "yes",
                                                                          progres_relationshiptofpname %in% c("Focal Point") & 
                                                                            3 %in% c(1698) ~ "dk", TRUE, ~ NA_character_),
                ...
                i.hh_children_worked_Hhchores = case_when(progres_relationshiptofpname %in% c("Focal Point") & 124 %in% c(1694) ~ "yes",
                                                          ...
                                                          TRUE ~ NA_character_),
                i.avg_time_children_worked_HHchores = case_when(progres_relationshiptofpname %in% c("Focal Point") & 54 %in% c(1759, 2488) ~ "Between 1 and 14 hours",
